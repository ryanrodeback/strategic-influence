"""FixedMinimaxAgent: Corrected minimax with alpha-beta pruning and time limits.

Key improvements over MinimaxAgent:
1. CRITICAL FIX: Corrected depth tracking in _max_player → _min_opponent call
2. Time limits to prevent hangs
3. Transposition table for caching evaluations
4. Better move generation limiting
5. Iterative deepening for time-bounded search

The original MinimaxAgent has a bug where _max_player passes wrong depth
to _min_opponent (line 391: should be `depth - 1` not `depth`), causing
infinite recursion. This version fixes that and adds optimizations.
...
class FixedMinimaxAgent:
    """Minimax agent with corrected depth tracking and optimizations.

    Key parameters:
    - max_depth: How far to look ahead (1-3 recommended)
    - max_moves: Limit on moves to consider (15-25)
    - max_candidates_per_territory: Limit candidates per territory (3-5)
    - time_limit_sec: Hard time limit per move (5-10 seconds)
    ...
    def __init__(
        ...
        # Transposition table: maps (board_hash, depth, player) -> evaluation
        self._transposition_table = {}
        self._tt_hits = 0

    @property
    def name(self) -> str:
        return f"FixedMinimax(d={self._max_depth})"
    ...
    def choose_actions(
        ...
        """Choose best move using minimax with alpha-beta and time limits."""
        ...
        if self._verbose:
            print(f"  FixedMinimax: {len(moves)} moves, depth {self._max_depth}")
        ...
        for move in moves:
            ...
            if time.time() - self._start_time > self._time_limit_sec:
                if self._verbose:
                    print(f"  Time limit reached, using best move found")
                break

            value = self._min_opponent(
                state, player, move, self._max_depth, alpha, beta, config
            )
        ...
        if self._verbose:
            ...
            print(f"  FixedMinimax: {self._nodes_searched} nodes, {self._nodes_pruned} pruned ({prune_rate:.0%}), TT hits: {self._tt_hits}, {self._last_search_time:.2f}s")
    ...
    def _board_hash(self, state: GameState) -> int:
        """Simple zobrist-style hash of board state."""
        h = 0
        for pos in state.board.positions:
            owner = state.board.get_owner(pos)
            stones = state.board.get_stones(pos)
            # Simple hash: combine owner and stone count
            h ^= hash((pos, owner, stones))
        return h
    ...
    def _generate_limited_moves(
        ...
        """Generate limited set of moves quickly."""
    ...
    def _get_limited_options(
        ...
        # Select best neighbors to consider
    ...
    def _max_player(
        ...
        for i, move in enumerate(moves):
            # FIX: Pass depth - 1 to _min_opponent (was wrong in original)
            value = self._min_opponent(
                state, player, move, depth - 1, alpha, beta, config
            )
